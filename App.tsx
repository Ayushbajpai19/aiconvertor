...
const App: React.FC = () => {
  ...
  const handleConvertAll = async () => {
    ...
    if (allTransactions.length > 0) {
      ...
      if (processingErrors.length > 0) {
          ...
          setError("We successfully processed your document(s), but couldn't find any transactional data. This can happen if the document is not a bank statement or has an unsupported format.");
      } else {
          setError("No transactions could be extracted from the selected files. Please check the files and try again.");
      }
    ...
    <div className="w-full max-w-6xl mx-auto animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 px-1">
        <div>
          ...
          <p className="text-gray-500 mt-1">Extracted {transactions.length} transactions from {files.filter(f => f.status === 'success' && transactions.some(t => t.sourceFile === f.file.name)).length} statement(s).</p>
        </div>
  ...
  const renderContent = () => {
    switch (appState) {
      case AppState.IDLE:
        return <FileUpload onFilesSelect={handleFilesSelect} disabled={false} />;
  ...
  return (
    <div className="min-h-screen bg-gray-50 apple-gradient text-gray-800">
      <div className="container mx-auto px-4 py-8 sm:py-16">
        ...
        <footer className="text-center mt-16">
            <p className="text-sm text-gray-500">&copy; {new Date().getFullYear()} Acme Corporation. All Rights Reserved.</p>
        </footer>
      </div>
    </div>
  );
